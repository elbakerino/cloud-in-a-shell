...
DYN_SERV_INFO=$(echo "show servers state ${SERV_BACKEND}" | socat stdio /var/run/haproxy.sock | grep -F "${SERV_NAME} ")
...
TMP=$(echo "set server ${SERV_BACKEND}/${SERV_NAME} state maint" | socat stdio /var/run/haproxy.sock)
...
if [ ${srv_admin_state} = 4 ]; then
  ...
  echo " ✓ Server ${SERV_BACKEND}/${SERV_NAME} is disabled"
elif [ ${srv_admin_state} = 12 ]; then
  echo " X Server ${SERV_BACKEND}/${SERV_NAME} is drained"
fi
